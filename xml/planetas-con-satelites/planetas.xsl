...
<xsl:template match="/">
	...
	<body>
		<div id="container">
			...
			<div id="formulario">
				<header>
					<h2>PLANETAS CON SUS SATÉLITES (MÁS GRANDES)</h2>
				</header>
				...
				<table>
					...
					<xsl:for-each select="/sistema-solar/planeta[not(@enano)]">
					<xsl:sort select="distancia" data-type="number" order="ascending"/>
						<tr class="planeta">
							<td><xsl:value-of select="nombre"/></td>
							<td><xsl:value-of select="distancia"/> (millones de km)</td>
							<td><xsl:value-of select="numero-satelites"/></td>
						</tr>
						<xsl:for-each select="./satelite">
						<xsl:sort select="distancia" data-type="number" order="ascending"/>
							<tr class="satelite">
								<td style="padding-left:20px;">- <xsl:value-of select="nombre"/></td>
								<td colspan="2"><xsl:value-of select="distancia"/> (miles de km)</td>
							</tr>
						</xsl:for-each>
					</xsl:for-each>
				</table>
